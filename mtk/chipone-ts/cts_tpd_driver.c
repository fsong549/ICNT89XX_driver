...
static int chipone_tpd_local_init(void)
{
    ...
    int ret;
	cts_info("chipone_tpd_local_init ... enter");
	
	tpd->reg=regulator_get(tpd->tpd_dev, "vtouch"); // get pointer to regulator structure
	if (IS_ERR(tpd->reg)) {
		cts_err("[BND][Touch]regulator_get() failed!");
	}

	ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);	// set 2.8v
	cts_info("regulator_set_voltage ret = : %d", ret);	
	if (ret){
		cts_err("[BND][Touch] regulator_set_voltage() failed!");
	}
	...
	cts_info("regulator_enable ret = : %d", ret);	
	if (ret != 0){
		cts_err("Failed to disable reg-vgp2: %d", ret);	
	}
	msleep(100);
	
    if ((ret = cts_i2c_driver_init()) != 0) {
        cts_err("Init i2c driver failed %d", ret);
        return ret;
    }

    if (tpd_load_status == 0) {
        cts_err("I2C driver not load successfully, exit.");
        cts_i2c_driver_exit();
        return -1;
    }
...
static void chipone_tpd_suspend(struct device *dev)
{
	...
	ret = regulator_disable(tpd->reg);
	if (ret != 0){
		cts_err("Failed to disable reg-vgp2: %d\n", ret);
	}
...
static int __init chipone_tpd_driver_init(void)
{
    ...
    cts_info("Chipone CTS TPD driver %s", CFG_CTS_DRIVER_VERSION);
...
MODULE_DESCRIPTION("Chipone TPD Driver for MTK platform");
